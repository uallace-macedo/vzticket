...
async def test_repository_create_claim_token_success(claim_repository, user):
    ...
    assert saved_token.type == ClaimType.DEPOSIT
    assert saved_token.status == ClaimTokenStatus.PENDING


async def test_repository_get_by_token_success(claim_repository, user):
    now = datetime.now(timezone.utc)
    token_str = str(uuid.uuid4())
    ...
    found_token = await claim_repository.get_by_token(token_str)
